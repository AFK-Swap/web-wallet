...
export default function NotificationsPage() {
  ...
  const handleAccept = async (notification: Notification) => {
    try {
      if (notification.type === 'credential-share') {
        // OPTIMISTIC UPDATE: Remove notification immediately for instant UI response
  ...
  const handleReject = async (notification: Notification) => {
    try {
      // OPTIMISTIC UPDATE: Remove notification immediately for all types
      console.log(`🚫 Canceling notification: ${notification.id}`);
      setNotifications(prev => prev.filter(n => n.id !== notification.id));

      // Delete the notification from backend completely (don't wait for response)
      fetch(`/api/notifications/${notification.id}`, {
        method: 'DELETE'
      }).then(response => {
        if (response.ok) {
          console.log(`✅ Notification ${notification.id} deleted from backend`);
        } else {
