...
import { IssuerConnection } from '@/components/IssuerConnection'
...
export default function Dashboard() {
  ...
  const [showIssuerConnection, setShowIssuerConnection] = useState(false)
  ...
  const handleDIDCommConnection = (connectionId: string) => {
    ...
    console.log('DIDComm connection (Verifier) established:', connectionId)
  }

  const handleIssuerConnection = (connectionId: string) => {
    setActiveConnections(prev => [...prev, connectionId])
    localStorage.setItem('web_wallet_connections', JSON.stringify([...activeConnections, connectionId]))
    setShowIssuerConnection(false)
    console.log('Issuer connection established (NO notification):', connectionId)
  }
  ...
  return !isAuthenticated ? (
    ...
    <div style={{
            ...
            <div style={{
              ...
              {(() => {
                ...
                return [
                  ...
                  createAppCard(
                    null,
                    () => setShowIssuerConnection(true),
                    <svg style={{ width: '48px', height: '48px', color: 'white', marginBottom: '12px', opacity: 0.9 }} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>,
                    'Connect Issuer',
                    'Get Credentials'
                  ),
      ...
      {/* Issuer Connection Modal (No Notification) */}
      {showIssuerConnection && (
        <IssuerConnection
          onConnectionEstablished={handleIssuerConnection}
          onClose={() => setShowIssuerConnection(false)}
        />
      )}

      {/* DIDComm Connection Modal (Verifier - Creates Notification) */}
